...
// CREATE
// search by state
router.get('/addByState/:state', function(req, res) {
...
router.get('/legislators/query/last_name=:last_name', function(req, res, next) {
    var last_name_query = req.params.last_name;
    var legislators = Legislator.find({
...
router.get('/legislators/query/state=:state', function(req, res, next) {
    var legislators = Legislator.find({
...
router.get('/legislators/query/party=:party', function(req, res, next) {
    var legislators = Legislator.find({
...
function populateData(bioguide_id, crp_id) {
    var legislator_ids = [];
...
function addBills(bioguide_ids) {
    console.log('inside getbills w ' + bioguide_ids.length);
    for (var i = 0; i < bioguide_ids.length; i++) {
        var bio = bioguide_ids[i];
            ...
            if (!error && response.statusCode == 200) {
                ...
                for (var j = 0; j < results.length; j++) {
                    var bill = {
                        ...
                        vetoed: results[j].history.active
                    };
                    bills.push(bill);
                }
                // console.log(bills);
